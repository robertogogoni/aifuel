#!/bin/bash
# aifuel-history.sh — Usage history tracking with sparkline visualization
# Sourced by aifuel.sh and aifuel-tui.sh

# History directory
AIFUEL_HISTORY_DIR="$HOME/.cache/aifuel/history"

_ensure_history_dir() {
    [ -d "$AIFUEL_HISTORY_DIR" ] || mkdir -p "$AIFUEL_HISTORY_DIR" 2>/dev/null
}
...
record_snapshot() {
    ...
    local history_file="$AIFUEL_HISTORY_DIR/${provider}.jsonl"
    local ts
    ts=$(date +%s)
    local line
    ...
    local retention_days="${AIFUEL_HISTORY_RETENTION:-7}"
    local cutoff
