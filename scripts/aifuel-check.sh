#!/bin/bash
# aifuel check — Diagnostic tool for aifuel
...
if [ -f "$AIFUEL_CONFIG" ]; then
    if jq empty "$AIFUEL_CONFIG" 2>/dev/null; then
        ok "config file valid JSON ($AIFUEL_CONFIG)"
    else
        fail "config file is malformed JSON ($AIFUEL_CONFIG)"
    fi
else
...
if [ -d "$AIFUEL_LOG_DIR" ]; then
    ok "log directory exists ($AIFUEL_LOG_DIR)"
else
    mkdir -p "$AIFUEL_LOG_DIR" 2>/dev/null
    if [ -d "$AIFUEL_LOG_DIR" ]; then
        ok "log directory created ($AIFUEL_LOG_DIR)"
    else
        fail "could not create log directory ($AIFUEL_LOG_DIR)"
    fi
fi

if [ -f "$AIFUEL_LOG_FILE" ]; then
    lines=$(wc -l < "$AIFUEL_LOG_FILE")
