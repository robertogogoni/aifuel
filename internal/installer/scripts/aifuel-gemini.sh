...
AIFUEL_PROVIDER="gemini"
CACHE_FILE="$AIFUEL_CACHE_DIR/aifuel-cache-gemini.json"
...
CLIENT_CREDS_CACHE="$HOME/.config/aifuel/gemini-client-creds.json"
...
if [ "$now_ms" -ge "$expiry_date" ] && [ -n "$refresh_token" ]; then
    ...
    if [ $? -ne 0 ] || [ -z "$refresh_response" ]; then
        ...
        error_json "token refresh failed (network error)" "check internet or run 'aifuel check'"
    fi
...
if [ $? -ne 0 ] || [ -z "$quota_response" ]; then
    ...
    error_json "quota API request failed" "check internet or run 'aifuel check'"
fi
