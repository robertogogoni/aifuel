#!/bin/bash
# AIFuel TUI — Interactive dashboard with gum
# Launched via waybar click or terminal
...
ensure_config() {
    mkdir -p "$(dirname "$AIFUEL_CONFIG")"
    if [ ! -f "$AIFUEL_CONFIG" ]; then
        cat > "$AIFUEL_CONFIG" << 'EOF'
{
  "display_mode": "icon",
  "refresh_interval": 60,
...
detect_system_theme() {
    local theme_pref
    theme_pref=$(jq -r '.theme // "auto"' "$AIFUEL_CONFIG" 2>/dev/null)
...
fetch_all() {
    ...
    clear
    echo ""
    gum style --foreground 39 --bold "  󰧑  Refreshing AIFuel data..."
    ...
    if [ "$claude_enabled" = "true" ]; then
        CLAUDE_JSON=$("$LIBEXEC_DIR/aifuel-claude.sh" 2>/dev/null)
    ...
    if [ "$codex_enabled" = "true" ]; then
        CODEX_JSON=$("$LIBEXEC_DIR/aifuel-codex.sh" 2>/dev/null)
    ...
    if [ "$gemini_enabled" = "true" ]; then
        GEMINI_JSON=$("$LIBEXEC_DIR/aifuel-gemini.sh" 2>/dev/null)
...
show_settings() {
    while true; do
        ...
        current_mode=$(jq -r '.display_mode // "icon"' "$AIFUEL_CONFIG")
        ...
        local choice
        local notify_on
        notify_on=$(jq -r '.notifications_enabled // true' "$AIFUEL_CONFIG")
        local notify_mark
        ...
        case "$choice" in
            d)
                ...
                if [ -n "$new_mode" ]; then
                    local updated
                    updated=$(jq --arg m "$new_mode" '.display_mode = $m' "$AIFUEL_CONFIG")
            ...
            i)
                ...
                if [ -n "$new_interval" ]; then
                    local updated
                    updated=$(jq --argjson i "$new_interval" '.refresh_interval = $i' "$AIFUEL_CONFIG")
                    atomic_write "$AIFUEL_CONFIG" "$updated"
                    refresh_waybar
                fi
                ;;
            t)
                ...
                if [ -n "$new_ttl" ]; then
                    local updated
                    updated=$(jq --argjson t "$new_ttl" '.cache_ttl_seconds = $t' "$AIFUEL_CONFIG")
                    atomic_write "$AIFUEL_CONFIG" "$updated"
                    refresh_waybar
                fi
                ;;
            e)
                ...
                if [ -n "$new_theme" ]; then
                    local updated
                    updated=$(jq --arg t "$new_theme" '.theme = $t' "$AIFUEL_CONFIG")
                    atomic_write "$AIFUEL_CONFIG" "$updated"
                    CURRENT_THEME=$(detect_system_theme)
                    apply_theme "$CURRENT_THEME"
                    refresh_waybar
            ...
            n)
                ...
                local updated
                updated=$(jq --argjson v "$new_val" '.notifications_enabled = $v' "$AIFUEL_CONFIG")
                atomic_write "$AIFUEL_CONFIG" "$updated"
                ;;
            c)
                local new_val
                ...
                local updated
                updated=$(jq --argjson v "$new_val" '.providers.claude.enabled = $v' "$AIFUEL_CONFIG")
                atomic_write "$AIFUEL_CONFIG" "$updated"
                refresh_waybar
                ;;
            x)
                ...
                local updated
                updated=$(jq --argjson v "$new_val" '.providers.codex.enabled = $v' "$AIFUEL_CONFIG")
                atomic_write "$AIFUEL_CONFIG" "$updated"
                refresh_waybar
                ;;
            g)
                ...
                local updated
                updated=$(jq --argjson v "$new_val" '.providers.gemini.enabled = $v' "$AIFUEL_CONFIG")
                atomic_write "$AIFUEL_CONFIG" "$updated"
                refresh_waybar
                ;;
            a)
                ...
                local updated
                updated=$(jq --argjson v "$new_val" '.providers.antigravity.enabled = $v' "$AIFUEL_CONFIG")
                atomic_write "$AIFUEL_CONFIG" "$updated"
                refresh_waybar
                ;;
            p)
...
_show_log_pager() {
    local filter="$1"
    local log_file="$AIFUEL_LOG_FILE"
...
show_history() {
    ...
    for i in "${!providers[@]}"; do
        local p="${providers[$i]}"
        local n="${names[$i]}"
        local history_file="$AIFUEL_HISTORY_DIR/${p}.jsonl"
...
copy_to_clipboard() {
    ...
    local report
    report="AIFuel Report ($(date '+%Y-%m-%d %H:%M'))"
    report+=$'\n'"──────────────────────────────────"
...
main() {
    ...
    while true; do
        ...
        case "$choice" in
            r)
                rm -f "$AIFUEL_CACHE_DIR"/aifuel-cache-*.json
                fetch_all
                ;;
            h)
                show_history
