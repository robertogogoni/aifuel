...
AIFUEL_PROVIDER="codex"
CACHE_FILE="$AIFUEL_CACHE_DIR/aifuel-cache-codex.json"
...
try_rpc() {
    ...
    local init_req='{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"clientInfo":{"name":"aifuel-waybar","version":"1.0.0"}}}'
    echo "$init_req" >&3
...
try_api() {
    ...
    # Build curl headers
    local curl_args=(-s "$USAGE_API_URL" -H "Authorization: Bearer $token" -H "User-Agent: aifuel-waybar")
...
# Both methods failed
error_json "both RPC and API methods failed" "ensure codex is installed and authenticated; run 'aifuel check'"
